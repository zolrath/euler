...
func getBlackslash(y, x int) int {
	//  Backslash = \
	product := 1
	if x > 16 || y > 16 {
		return product
...
func getForwardslash(y, x int) int {
	//  Forwardslash = /
	product := 1
	if x < 3 || y < 3 {
		return product
...
func Euler011() int {
	grid = [][]int{[]int{8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8},
	...
	for y := 0; y < 16; y++ {
		for x := 0; x < 16; x++ {
			checkIfGreatest(getUpDown(y, x))
			checkIfGreatest(getLeftRight(y, x))
			checkIfGreatest(getBlackslash(y, x))
			checkIfGreatest(getForwardslash(y, x))
		}
	}
	return greatest
}
